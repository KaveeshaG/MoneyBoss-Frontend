...
namespace MonyBoss
{
    public partial class mainScrn : Form
    {
        ...
        void ReloadIncome()
        {
            ...
            var expenseTransactions = DbContext.GetInstance().GetCollection<ExpenseTransaction>("expense_transactions")
                        ...
                        .Include(x => x.Category)
                          .FindAll();
            //Filter the Search
            incomeTransactions = incomeTransactions.Where(r => r.PaymentFrom.ToLower().Contains(inputSearch.Text.ToLower())
            || r.Desciption.ToLower().Contains(inputSearch.Text.ToLower())
            || r.TransactionCode.ToLower().Contains(inputSearch.Text.ToLower())
            );
            ...
            var incAve=incomeTransactions.Sum(r => r.Amount);
            var expAve=expenseTransactions.Sum(r => r.Amount);
            ProfitNo.Text = (incAve - expAve).ToString();
        ...
        void ReloadExpenses()
        {
            ...
            //Filter the Search
            expenseTransactions = expenseTransactions.Where(r => r.PaymentTo.ToLower().Contains(inputSearch.Text.ToLower())
            || r.Desciption.ToLower().Contains(inputSearch.Text.ToLower())
            || r.TransactionCode.ToLower().Contains(inputSearch.Text.ToLower())
            );
        ...
        */
        ...
        {
            IncomeId = Convert.ToInt32(GridIncome.SelectedRows[0].Cells[0].Value);

        }

        private void GridExpense_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {

        }

        private void deleteBtnInc_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Confirm delete transaction", "MoneyBoss", MessageBoxButtons.YesNoCancel) != DialogResult.Yes)
            {
                bunifuSnackbar1.Show(this, "Transaction deletion Cancelled", Bunifu.UI.WinForms.BunifuSnackbar.MessageTypes.Warning, 2000);
                return;
            }
            ...
            if (IncomeId > 0)
            {
                var incomeTransactions = DbContext.GetInstance().GetCollection<IncomeTransaction>("income_transaction")
                    .Include(x => x.Account)
                    .Include(x => x.Category)
                    .Delete(IncomeId);

                bunifuSnackbar1.Show(this, "Deleted", Bunifu.UI.WinForms.BunifuSnackbar.MessageTypes.Success, 2000);
                return;
            }

            bunifuSnackbar1.Show(this, "Select the Transaction", Bunifu.UI.WinForms.BunifuSnackbar.MessageTypes.Warning, 2000);
            return;

            //DbContext.GetInstance().GetCollection<IncomeTransaction>("income_transactions").Delete(incomeTransaction.Id);
            //IncomeId = 0;
        }

        private void UpdateBtnClick (object sender, EventArgs args)
        {
            if (IncomeId == 0)
            {
                // Popup a dialog box which shows no row has selected
                return;
            }

            // 1. Pass IncomeId value to Update popup form through Form constructor
            // 2. Get the object by IncomeId from the DB
            // 3. Set object values to text fields 
            // 4. Let user to edit fileds
            // 5. Save edited values to DB

            /**

            var incomeTransactions = DbContext.GetInstance().GetCollection<ExpenseTransaction>("income_transactions")
                                       .Include(x => x.Account)
                                     .Include(x => x.Category)
                                       .FindById(IncomeId);
            **/
        ...
        private void IncomeNo_Click(object sender, EventArgs e)
        {
